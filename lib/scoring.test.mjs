...
test('K: FG by distance bucket + PAT (Ka\'imi Fairbairn, 2025)', () => {
  // 3*(0+8+11) + 3*16 + 4*9 + 1*28 = 57 + 48 + 36 + 28 = 169
  const cols = {
    [COL.fg0_19]: '0', [COL.fg20_29]: '8', [COL.fg30_39]: '11',
    [COL.fg40_49]: '16', [COL.fg50]: '9', [COL.pat]: '28',
  };
  near(pointsFor(cols, 'K', scoring).total, 169);
});

test('full-PPR gives receptions exactly 1 each', () => {
...
test('QB line reproduces Yahoo (Dak Prescott, 2025)', () => {
  ...
  // 4837/25=193.48 + 38*6=228 + 12*-1=-12 + 90/10=9 + 12 + 5*-2=-10
  near(pointsFor(cols, 'QB', scoring).total, 420.48);
});
